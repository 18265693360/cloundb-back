        ...
        if (phoneStatus) {
            ...
            if (user) {//用户已经注册
                ...
                if (msg) { //判断是否已经发送过验证码 第一行判断是否正确
                    //第二行判断是否 过期
                    let msgData = new Data(msg.updataTime);
                    let msgTime = Math.round(msgData.getTime() / 1000);
                    let nowTime = Math.round(Data.now() / 1000);
...
async function getUserById(req, res, next){
    try {
        ...
        const userData = await userModel.findById(mongoose.Types.ObjectId(userId)).select('-password'); //限制密码
        res.json({
                code: 200,
                data: userData
        })
